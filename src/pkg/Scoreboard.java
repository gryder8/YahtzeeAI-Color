...
class Scoreboard {
    ...
    void setScoreForAllDice(AllDice gameDice, int scoreChoiceIndex) { //take the validated user input and pass it into the current scorecard
    ...
    Scoreboard() {
        setup();
    } //constructor create object array
    ...
    void printTemporaryScorecardForGameDice(AllDice gameDice) { //using the tempValues array, generate a temporary scorecard of score options
    ...
    void printFinalScorecardForGameDice() { //print the final scorecard that was generated by generateScorecard()
    ...
    void printCurrentScorecardForGameDice() { //print the current scorecard that was generated by generateScorecard()
    ...
    private String generateScorecard(Score[] creationArray) { //create a scorecard given an array of Score objects
        String scoreString = "";
        totalPlayerScore = 0; //start at 0 each time
        for (int i = 0; i < creationArray.length; i++) {
            if (creationArray[i] != null) { //make sure to avoid NPE by checking if the object has been removed b/c it's taken (making that index null)
                scoreString += i + ") " + creationArray[i].getName() + ": " + creationArray[i].getValue() + "\n"; //append everything into as
                totalPlayerScore += creationArray[i].getValue(); //add the value to the total score
            }
        }
        return scoreString;
    }

    boolean isUserScoreChoiceAvailable(int ScoreChoiceIndex) { //check if the validated user input is an open choice
    ...
    boolean isBoardFilled() { //check to see if the board is filled by walking through the array and using .getIsUsed()
